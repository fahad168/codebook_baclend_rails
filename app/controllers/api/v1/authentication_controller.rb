class Api::V1::AuthenticationController < Api::V1::ApiController
  ...
  def login
    ...
    return render json: { message: "User not found" }, status: :unprocessable_entity unless @user
  ...
  def logout
    ...
    verification_token = @current_user.authentication_tokens.find_by_token(token)
    if verification_token
      @current_user.authentication_tokens.find_by_token(token).destroy
